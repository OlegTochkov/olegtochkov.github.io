...
function t1093__getBlockById(elementId) {
  var pairedRecord = document.getElementById('rec' + elementId) || document.getElementById(elementId);
  if (!pairedRecord) {
  ...
  var recordType = pairedRecord.getAttribute('data-parenttplid') || pairedRecord.getAttribute('data-record-type');
...
function t1093__showOnClick(e) {
  ...
  if (lastOpenedPopupHook) {
    ...
    var shouldClosePopup = lastOpenedPopup && lastOpenedPopup.getAttribute('data-popup-close-on-open-another') === 'true';
    if (shouldClosePopup) {
...
function t1093__closePopup_isLastPopupWithLockedScroll() {
  var popups = t1093__getPopupObjValue('openPopUpList').map(hook => document.querySelector('[data-tooltip-hook="' + hook + '"]')).filter(Boolean);
